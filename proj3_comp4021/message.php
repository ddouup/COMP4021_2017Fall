    ...
    <head>
        ...
        function updateChat(xmlDoc) {
            ...
            for (i = lastMsgID; i < messages.length; i++){
                showMessage(messages[i].getAttribute("name"), messages[i].textContent, messages[i].getAttribute("color"));
            }
        ...
        function showMessage(nameStr, contentStr, colorStr){
                ...
                contentNode.style.setProperty("fill", colorStr, null);
                contentStr = contentStr.replace(/(https{0,1}\:\/\/[^\s]*)/g,
                             '<a style = "text-decoration:underline; fill:'+ colorStr +'"target="blank" xlink:href="$1">$1</a>');
